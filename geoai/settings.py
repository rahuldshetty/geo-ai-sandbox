...
Settings are runtime-editable (the Settings dialog): model, message retention, and
...
from .config import DEFAULT_MODEL, keep_messages, model_from_env

SETTINGS_FILE = Path(__file__).resolve().parent.parent / "settings.json"

_THEMES = ("light", "dark")
_DEFAULT_KEEP_MESSAGES = 24
...
def _normalize(settings: dict) -> dict:
    ...
    try:
        keep = int(settings.get("keep_messages", _DEFAULT_KEEP_MESSAGES))
    except (TypeError, ValueError):
        keep = _DEFAULT_KEEP_MESSAGES
    theme = settings.get("theme") if settings.get("theme") in _THEMES else "light"
    return {
        "model": model,
        "keep_messages": max(0, keep),
        "theme": theme,
